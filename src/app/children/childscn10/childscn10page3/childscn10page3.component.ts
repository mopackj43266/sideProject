...
export class Childscn10page3Component implements OnInit {
  ...
  data_number3(x: string) {
    if (x != null) {
      if (x.includes('.')) {
        return this.f01008scn4Service.toCurrency(this.f01008scn4Service.toNumber(x.split('.')[0])) + '.' + x.split('.')[1];
      } else {
        return this.f01008scn4Service.toCurrency(this.f01008scn4Service.toNumber(x))
      }
    } else {
      return x;
    }
