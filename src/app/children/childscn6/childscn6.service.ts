...
export class Childscn6Service extends BaseService {
  ...
  getR20List(baseUrl:string ,json: JSON): Observable<any> {
    return this.postJsonObject(baseUrl, json);
  }
  getAAS008List(json: JSON): Observable<any> {
    const baseUrl = 'f01/childscn6action4';
    return this.postJsonObject(baseUrl, json);
  }
  getVAM020List(json: JSON): Observable<any> {
    const baseUrl = 'f01/childscn6action5';
    return this.postJsonObject(baseUrl, json);
  }
  getRAS020List(json: JSON): Observable<any> {
  ...
  getTableData(code: JCICCode): JCICTable {
    ...
    if (code === JCICCode.BAM061) {
      dataList = [
        ...
        { name: '年薪(千元)', bodyKey: 'SALARY', type: 'number' },
      ];
    ...
    if (code === JCICCode.KRM043) {
      dataList = [
        ...
        { name: '年薪/營業額(千元)', bodyKey: 'SALARY', width: '200px', type: 'number' },
    ...
    if (code === JCICCode.BAM062) {
      dataList = [
        ...
        { name: '年收入(仟元)', bodyKey: 'SALARY', type: 'number' },
    ...
    if (code === JCICCode.BAM501) {
      dataList = [
        { name: '行庫代號', bodyKey: 'MAIN_CODE' },
        { name: '行庫名稱', bodyKey: 'BANK_NAME' },
        { name: '無擔保放款(仟元)', bodyKey: 'B_AMT', type: 'number' },
        { name: '現金卡放款(仟元)', bodyKey: 'Y_AMT' , type: 'number'},
        { name: '信用卡循環信用(仟元)', bodyKey: 'C1_AMT', type: 'number' },
        { name: '未到期待付款(仟元)', bodyKey: 'C2_AMT', type: 'number' },
        { name: '', bodyKey: '', width: '30%' },
      ];
    ...
    if (code === JCICCode.BAM502) {
      dataList = [
        ...
        { name: '未逾期金額(仟元)', bodyKey: 'LOAN_AMT', type: 'number' },
        { name: '逾期金額(仟元)', bodyKey: 'PASS_DUE_AMT', type: 'number' },
      ];
    ...
    if (code === JCICCode.BAM504) {
      dataList = [
        ...
        { name: '行庫代號', bodyKey: 'BANK_CODE' },
        { name: '行庫名稱', bodyKey: 'BANK_NAME' },
        { name: '金額(千元)', bodyKey: 'AMT', type: 'number' },
        { name: '', bodyKey: '', width: '30%' },
      ];
    ...
    if (code === JCICCode.BAM505) {
      dataList = [
        ...
        { name: '行庫代號', bodyKey: 'BANK_CODE' },
        { name: '行庫名稱', bodyKey: 'BANK_NAME' },
        { name: '新增核准額度訂約金額(千元)', bodyKey: 'CONTRACT_AMT' , type: 'number'},
        { name: '應計入DBR22倍規範之金額(千元)', bodyKey: 'DBR22_AMT' , type: 'number'},
        { name: '', bodyKey: '', width: '30%' },
      ];
    ...
    if (code === JCICCode.BAM032) {
      dataList = [
        ...
        { name: '行庫代號', bodyKey: 'BANK_CODE' },
        { name: '行庫名稱', bodyKey: 'BANK_NAME' },
        { name: '餘額增加金額(千元)', bodyKey: 'PAY_AMT' , type: 'number'},
        { name: '最新授信餘額(千元)', bodyKey: 'BAL_AMT', type: 'number' },
    ...
    if (code === JCICCode.BAM033) {
      dataList = [
        ...
        { name: '行庫代號', bodyKey: 'BANK_CODE' },
        { name: '行庫名稱', bodyKey: 'BANK_NAME' },
        { name: '餘額減少金額（千元）', bodyKey: 'PAY_AMT', type: 'number' },
        { name: '最新授信餘額(千元)', bodyKey: 'BAL_AMT', type: 'number' },
        { name: '授信科目', bodyKey: 'ACCOUNT_CODE' },
        { name: '科目別註記', bodyKey: 'ACCOUNT_CODE2' },
        { name: '授信科目名稱', bodyKey: 'ACCOUNT_NAME' },
    ...
    if (code === JCICCode.BAS010) {
      dataList = [
        { name: '最新授信總餘額（千元）', bodyKey: 'TOT_BAL_AMT', type: 'number' },
    ...
    if (code === JCICCode.BAM029) {
      dataList = [
        ...
        { name: '清償時最新授信月底餘額-未逾期金額(千元)', bodyKey: 'LASTMON_LOAN_AMT', width: '300px', type: 'number' },
        { name: '清償時最新授信月底餘額-逾期金額(千元)', bodyKey: 'LASTMON_PASS_DUE_AMT', width: '300px', type: 'number' },
        { name: '訂約金額(千元)', bodyKey: 'NEW_CONTRACT_AMT', width: '150px', type: 'number' },
        { name: '當日動撥金額(千元)', bodyKey: 'NEW_LOAN_AMT', width: '150px' , type: 'number'},
        { name: '清償金額(千元)', bodyKey: 'NEW_PAY_LOAN_AMT', width: '150px', type: 'number' },
        { name: '應計入DBR22倍規範之金額(千元)', bodyKey: 'NEW_DBR22_AMT', width: '300px', nzRight: true , type: 'number'},
      ];
    ...
    if (code === JCICCode.BAM421) {
      dataList = [
        ...
        { name: '綜合額度金額(千元)', bodyKey: 'CONTRACT_AMT1', width: '150px', type: 'number' },
        { name: '分項額度金額(千元)', bodyKey: 'CONTRACT_AMT', width: '150px', type: 'number' },
        { name: '未逾期金額(千元)', bodyKey: 'LOAN_AMT', width: '170px', type: 'number' },
        { name: '逾期未還金額(千元)', bodyKey: 'PASS_DUE_AMT', width: '150px', type: 'number' },
        ...
        { name: '現金卡日報Ｙ科目之可動用額度(千元)', bodyKey: 'CONTRACT_AMT_Y', width: '300px', type: 'number' },
    ...
    if (code === JCICCode.BAM101) {
      dataList = [
        { name: '資料年度', bodyKey: 'DATA_YYY', width: '90px', nzLeft: true },
        { name: '資料月份', bodyKey: 'DATA_MM', width: '90px' },
        { name: '短期放款金額(千元)', bodyKey: 'WECDU_AMT', width: '150px' , type: 'number'},
        { name: '中期放款金額(千元)', bodyKey: 'HIGZ_AMT', width: '150px', type: 'number' },
        { name: '擔保放款金額(千元)', bodyKey: 'MNSTRK_AMT', width: '150px', type: 'number' },
        { name: '應收信用狀金額(千元)', bodyKey: 'J_AMT', width: '170px' , type: 'number'},
        { name: '押匯承兌金額(千元)', bodyKey: 'PQX_AMT', width: '150px' , type: 'number'},
        { name: '本票保證金額(千元)', bodyKey: 'F_AMT', width: '150px', type: 'number' },
        { name: '保證款項金額(千元)', bodyKey: 'L_AMT', width: '150px', type: 'number' },
        { name: '不含逾催呆總餘額(千元)', bodyKey: 'NORMAL_LOAN_AMT', width: '170px', type: 'number' },
        { name: '逾期金額(千元)', bodyKey: 'PASS_DUE_AMT', width: '120px', type: 'number' },
        { name: '催收金額(千元)', bodyKey: 'COLLECTION_AMT', width: '120px' , type: 'number'},
        { name: '呆帳金額(千元)', bodyKey: 'BAD_DEBT', width: '120px', nzRight: true, type: 'number' },
      ];
    ...
    if (code === JCICCode.BAM305) {
      dataList = [
        ...
        { name: '關係代號', bodyKey: 'REL_CODE', width: '90px' },
        { name: '訂約金額(千元)', bodyKey: 'CONTRACT_AMT', width: '150px' , type: 'number'},
        { name: '放款未逾期(千元)', bodyKey: 'LOAN_AMT', width: '200px', type: 'number' },
        { name: '逾期未還金額(千元)', bodyKey: 'PASS_DUE_AMT', width: '200px' , type: 'number'},
    ...
    if (code === JCICCode.BAM070) {
      dataList = [
        ...
        { name: '可動用額度(千元)', bodyKey: 'CONTRACT_AMT', type: 'number' },
        { name: '授信未逾期餘額(千元)', bodyKey: 'LOAN_AMT', type: 'number' },
        { name: '逾期餘額(千元)', bodyKey: 'PASS_DUE_AMT', type: 'number' },
        { name: '當日還款紀錄', bodyKey: 'PAY_CODE' },
        { name: '還款紀錄說明', bodyKey: 'PAY_NAME' },
        { name: '授信餘額列報1(千元)之原始金額(元)', bodyKey: 'AC_AMT' , type: 'number'},
      ];
    ...
    if (code === JCICCode.KRM046) {
      dataList = [
        ...
        { name: '額度（千元）', bodyKey: 'LIMIT', width: '120px', type: 'number' },
        ...
        { name: '風險損失金額（千元）', bodyKey: 'RISK', width: '200px' , type: 'number'},
    ...
    if (code === JCICCode.KRM048) {
      dataList = [
        ...
        { name: '永久額度(千元)', bodyKey: 'PERM_LIMIT', width: '150px', type: 'number' },
        { name: '臨時額度(千元)', bodyKey: 'TEMP_LIMIT', width: '150px', type: 'number' },
        { name: '預借現金額度(千元)', bodyKey: 'CASH_LIMIT', width: '180px', type: 'number' },
        { name: '本期應付帳款(元)', bodyKey: 'PAYABLE', width: '180px' , type: 'number'},
        { name: '本期預借現金(元)', bodyKey: 'CASH_LENT', width: '180px', type: 'number' },
        { name: '上期應付帳款(元)', bodyKey: 'LAST_PAYA', width: '180px', type: 'number' },
        { name: '上期循環信用(元)', bodyKey: 'REVOL_BAL', width: '180px', type: 'number' },
