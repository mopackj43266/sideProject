...
import { F01017Service } from './f01017.service';
interface sysCode {
  value: string;
  viewValue: string;
}
@Component({
  selector: 'app-f01017',
  templateUrl: './f01017.component.html',
  styleUrls: ['./f01017.component.css', '../../assets/css/f01.css']
})
export class F01017Component implements OnInit {

  constructor(
    private router: Router,
    private f01017Service: F01017Service,
  ...
  ngOnInit(): void {
    ...
    this.f01017Service.getSysTypeCode('CASE_TYPE').subscribe(data => {
    ...
    this.f01017Service.getEmpNo(jsonObject).subscribe(data => {
  ...
  getCaseList() {
    ...
    jsonObject['empNo'] = this.empNo;
    jsonObject['stepName'] = 'swcS4EmpNo';
    jsonObject['opid'] = '2660';
    ...
    this.f01017Service.getCaseList(jsonObject).subscribe(data => {
      if (this.i == 0) {
        this.productList = [];
        this.productList.push({ value: '', viewValue: '請選擇' })
      ...
      if (data.rspBody.size > 0) {
        ...
        this.newData = this.f01017Service.getTableDate(this.pageIndex, this.pageSize, this.cusinfoDataSource);
  ...
  getLockCase(swcApplno: string, swcNationalId: string, swcCustId: string) {
    ...
    this.f01017Service.getLockCase(jsonObject).subscribe(data => {
  ...
  saveCaseMemo(swcApplno: string, swcCaseMemo: string) {
    ...
    this.f01017Service.saveCaseMemo(jsonObject).subscribe(data => {
      msg = data.rspMsg;
    });
    setTimeout(() => {
  ...
  onQueryParamsChange(params: NzTableQueryParams): void {
    ...
    if (this.pageIndex !== pageIndex) {
      ...
      this.newData = this.f01017Service.getTableDate(pageIndex, this.pageSize, this.cusinfoDataSource);
      // this.getCaseList();
