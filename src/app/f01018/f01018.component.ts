...
import { F01018Service } from './f01018.service';
interface sysCode {
  value: string;
  viewValue: string;
}
@Component({
  selector: 'app-f01018',
  templateUrl: './f01018.component.html',
  styleUrls: ['./f01018.component.css']
})
export class F01018Component implements OnInit {

  constructor(
    private router: Router,
    private f01018Service: F01018Service,
  ...
  ngOnInit(): void {
    ...
    this.f01018Service.getSysTypeCode('CASE_TYPE').subscribe(data => {
    ...
    this.f01018Service.getEmpNo(jsonObject).subscribe(data => {
  ...
  getCaseList() {
    ...
    jsonObject['empNo'] = this.empNo;
    jsonObject['stepName'] = 'swcS3EmpNo';
    jsonObject['opid'] = '2680';
    ...
    this.f01018Service.getCaseList(jsonObject).subscribe(data => {
      if (this.i == 0) {
        this.productList = [];
        this.productList.push({ value: '', viewValue: '請選擇' })
      ...
      if (data.rspBody.size > 0) {
        ...
        this.newData = this.f01018Service.getTableDate(this.pageIndex, this.pageSize, this.cusinfoDataSource);
  ...
  getLockCase(swcApplno: string, swcNationalId: string, swcCustId: string) {
    ...
    this.f01018Service.getLockCase(jsonObject).subscribe(data => {
  ...
  saveCaseMemo(swcApplno: string, swcCaseMemo: string) {
    ...
    this.f01018Service.saveCaseMemo(jsonObject).subscribe(data => {
      msg = data.rspMsg;
    });
    setTimeout(() => {
  ...
  onQueryParamsChange(params: NzTableQueryParams): void {
    ...
    if (this.pageIndex !== pageIndex) {
      ...
      this.newData = this.f01018Service.getTableDate(pageIndex, this.pageSize, this.cusinfoDataSource);
      // this.getCaseList();
