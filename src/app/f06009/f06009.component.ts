...
import { F06009cancelComponent } from './f06009cancel/f06009cancel.component';

@Component({
  selector: 'app-f06009',
  templateUrl: './f06009.component.html',
  styleUrls: ['./f06009.component.css', '../../assets/css/f02.css']
})
export class F06009Component implements OnInit {
  ...
  ngOnInit(): void {
    // 查詢案件分類
    this.f06009Service.getSysTypeCode('STATUS_DETAIL').subscribe(data => {
    ...
    // if (this.check()) {
      ...
      this.f06009Service.getData(url, jsonObject).subscribe(data => {
        if (data.rspBody.size == 0) {
          this.f06009Service.confrimCom("查無資料!");
          this.clear();
          return;
        } else {
          ...
          this.loading = false;
          this.newData = this.f06009Service.getTableDate(this.pageIndex, this.pageSize, this.resultData);
        }
      })
    // } else {
    //   this.f06009Service.confrimCom("至少輸入一項查詢!");
    //   return;
    // }
  }
  ...
  onQueryParamsChange(params: NzTableQueryParams): void {
    ...
    if (this.pageIndex !== pageIndex) {
      if (this.firstFleg != 1) {
        ...
        this.newData = this.f06009Service.getTableDate(pageIndex, this.pageSize, this.resultData);
      }
    }
  }
  ...
  detail(id: string, nationalId: string, cuCname: string, custId: string) {
    ...
    this.f06009Service.getData(apiurl, jsonObject).subscribe(data => {
      if (data.rspMsg == "success" && data.rspBody == "儲存成功!") {
        ...
        //開啟徵審主畫面
        let safeUrl = this.f06009Service.getNowUrlPath("/#/F01002/F01002SCN1");
  ...
  cancel(applno: string, statusDesc: string) {
    const dialogRef = this.dialog.open(F06009cancelComponent, {
